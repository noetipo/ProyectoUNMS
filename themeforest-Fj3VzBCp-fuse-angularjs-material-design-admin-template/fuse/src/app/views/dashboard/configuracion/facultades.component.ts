...
interface FacultadDialogData { mode: 'new' | 'edit'; item?: any; }
...
@Component({
  ...
  template: `
    <div class="p-5 w-[460px] max-w-full">
      <h2 class="text-base font-semibold text-slate-800 mb-3">
        {{ data.mode === 'new' ? 'Nueva facultad' : 'Editar facultad' }}
      </h2>
      <form [formGroup]="form" (ngSubmit)="save()" class="flex flex-col gap-1">
        <div>
          <label class="form-label" for="fa-nombre">Nombre <span class="form-required">*</span></label>
          <mat-form-field appearance="outline" class="w-full" subscriptSizing="dynamic">
            <input matInput id="fa-nombre" formControlName="nombre" placeholder="Facultad de Medicina" />
          </mat-form-field>
        </div>
        <div>
          <label class="form-label" for="fa-codigo">Código</label>
          <mat-form-field appearance="outline" class="w-full" subscriptSizing="dynamic">
            <input matInput id="fa-codigo" formControlName="codigo" maxlength="20" placeholder="FM" />
          </mat-form-field>
        </div>
...
export class FacultadFormDialogComponent {
  private _fb = inject(FormBuilder);
  protected ref = inject(MatDialogRef<FacultadFormDialogComponent>);
  protected data = inject<FacultadDialogData>(MAT_DIALOG_DATA);

  protected form: FormGroup = this._fb.group({
    nombre: [this.data.item?.nombre ?? '', [Validators.required, Validators.minLength(2), Validators.maxLength(200)]],
    codigo: [this.data.item?.codigo ?? '', Validators.maxLength(20)],
  });

  save(): void {
    if (this.form.invalid) { this.form.markAllAsTouched(); return; }
    const v = this.form.value;
    this.ref.close({ nombre: (v.nombre ?? '').trim(), codigo: (v.codigo ?? '').trim() || undefined });
  }
}
      ...
      <div class="page-header">
        <div>
          <div class="breadcrumb">
            ...
            <span class="breadcrumb__current">Facultades</span>
          </div>
          <h1 class="page-title">Facultades</h1>
        </div>
        <button type="button" class="btn-dark" (click)="onNew()">
          <mat-icon svgIcon="plus" class="size-3.5" /> Nueva facultad
        </button>
      </div>
      ...
      <div class="page-toolbar">
        <form [formGroup]="filterForm">
          <div class="toolbar-search">
            ...
            <input formControlName="search" placeholder="Buscar facultad..." />
          </div>
        </form>
      ...
      <div class="page-content">
        @if (loading()) {
              ...
              <tbody>
                @for (item of visibleItems(); track item.id; let i = $index) {
                  <tr [class.anim-flash]="item.id === highlightedId()">
                    ...
                    <td class="font-mono text-[11px] text-slate-400">{{ item.codigo ?? '—' }}</td>
                ...
                @empty {
                  <tr><td colspan="5" class="text-center">
                    <div class="table-empty">
                      <mat-icon svgIcon="heroicons_outline:building-library" class="size-10 text-slate-200" />
                      <p class="table-empty__text">Sin facultades</p>
                      <p class="table-empty__subtext">Crea la primera con el botón "Nueva facultad"</p>
                    </div>
                  </td></tr>
                }
              </tbody>
          ...
          <div class="md:hidden divide-y divide-slate-100">
              ...
              <div class="px-5 py-3 anim-row-in" [class.anim-flash]="item.id === highlightedId()">
                <div class="flex items-start justify-between gap-3">
                  <div class="flex-1 min-w-0">
                    <p class="font-mono text-[11px] text-slate-400">{{ item.codigo ?? '—' }}</p>
            ...
            @empty { <div class="flex flex-col items-center gap-2 py-16"><p class="text-sm text-slate-400">Sin facultades</p></div> }
          </div>
        }
      </div>
...
export class FacultadesComponent implements OnInit, OnDestroy {
  private _http = inject(HttpClient);
  private _fb = inject(UntypedFormBuilder);
  private _dialog = inject(MatDialog);
  private _confirm = inject(ConfirmDialogService);
  private readonly base = environment.url + END_POINTS.configuracion.facultades;
  ...
  onNew(): void {
    const ref = this._dialog.open(FacultadFormDialogComponent, { ...DIALOG_BASE, width: '480px', panelClass: DIALOG_PANEL, data: { mode: 'new' } });
  ...
  onEdit(item: any): void {
    const ref = this._dialog.open(FacultadFormDialogComponent, { ...DIALOG_BASE, width: '480px', panelClass: DIALOG_PANEL, data: { mode: 'edit', item } });
    ref.afterClosed().subscribe((dto) => {
      if (!dto) return;
  ...
  onDelete(item: any): void {
    this._confirm.confirmDelete({ title: 'Eliminar facultad', message: `¿Desactivar "${item.nombre}"? (borrado lógico)` })
