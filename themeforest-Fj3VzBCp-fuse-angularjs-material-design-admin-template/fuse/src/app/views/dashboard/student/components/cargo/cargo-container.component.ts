...
import { Cargo, PaginatedResponse } from '../../models/student.models';
      ...
      <div class="page-header">
        <div>
          <div class="breadcrumb">
            ...
            <span class="breadcrumb__current">Cargos</span>
          </div>
          <h1 class="page-title">Cargos Laborales</h1>
        </div>
      ...
      <div class="page-toolbar">
        <form [formGroup]="filterForm">
          <div class="toolbar-search">
            ...
            <input formControlName="search" placeholder="Buscar cargo..." />
          </div>
        </form>
        @if (!loading()) {
          ...
          <div class="divide-y divide-slate-50">
            @for (_ of skeletonRows; track $index; let i = $index) {
              <div class="skeleton-row" [style.opacity]="1 - i * 0.1">
                ...
                <div class="anim-shimmer h-2 rounded shrink-0" [style.width.px]="160 + (i % 4) * 30"></div>
                  ...
                  <tr>
                    <td colspan="6" class="text-center">
                      <div class="table-empty">
                        <mat-icon svgIcon="briefcase" class="size-10 text-slate-200" />
                        <p class="table-empty__text">Sin cargos registrados</p>
                        <p class="table-empty__subtext">Crea el primero con el botón "Nuevo cargo"</p>
                      </div>
                    </td>
                  </tr>
                }
          ...
          <div class="md:hidden divide-y divide-slate-100">
            ...
            @empty {
              <div class="flex flex-col items-center gap-2 py-16 anim-fade-in">
                <mat-icon svgIcon="briefcase" class="size-10 text-slate-200" />
                <p class="text-sm text-slate-400">Sin cargos registrados</p>
              </div>
            }
          </div>
...
export class CargoContainerComponent implements OnInit, OnDestroy {
  private _svc     = inject(CargoService);
  private _confirm = inject(ConfirmDialogService);
  private _dialog  = inject(MatDialog);
  private _fb      = inject(UntypedFormBuilder);

  protected paginatedResponse = signal<PaginatedResponse<Cargo>>({ content: [], totalElements: 0, totalPages: 0, currentPage: 0, pageSize: 10 });
  protected visibleItems      = signal<Cargo[]>([]);
  ...
  onNew(): void {
    const ref = this._dialog.open(CargoNewComponent, { ...DIALOG_BASE, width: '480px', panelClass: DIALOG_PANEL });
  ...
  onEdit(item: Cargo): void {
    const ref = this._dialog.open(CargoEditComponent, { ...DIALOG_BASE, width: '480px', panelClass: DIALOG_PANEL, data: item });
    ref.afterClosed().subscribe(dto => {
      if (!dto) return;
  ...
  private _revealRows(items: Cargo[]): void {
    this._clearTimers();
