...
export interface DesignarJuradoSustentacionData { tesisId: string; estudiante: string; titulo: string; }

/** Modal del Coordinador para designar el Jurado de Sustentación (3 docentes; el 1.º es Presidente). */
@Component({
  selector: 'app-designar-jurado-sustentacion-dialog',
  standalone: true,
  imports: [CommonModule, FormsModule, MatDialogModule, MatButtonModule, MatIconModule],
  template: `
    <div class="w-[520px] max-w-[92vw] flex flex-col max-h-[85vh]">
      <div class="px-4 pt-4 pb-2 shrink-0">
        <h2 class="text-base font-bold text-slate-800">Designar Jurado de Sustentación</h2>
      ...
      <div class="px-4 flex-1 min-h-0 overflow-y-auto space-y-3">
        <p class="text-[12px] text-slate-500">Designa <b>3 docentes</b>. El <b>primero</b> actúa como Presidente del Jurado. No puede ser el asesor.</p>
        <div>
...
export class DesignarJuradoSustentacionDialogComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<DesignarJuradoSustentacionDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: DesignarJuradoSustentacionData,
  ) {
  ...
  designar(): void {
    ...
    this._svc.designarJuradoSustentacion$(this.data.tesisId, [this.m1, this.m2, this.m3]).subscribe({
      next: () => { this._toast.success('Jurado de Sustentación designado'); this.dialogRef.close(true); },
