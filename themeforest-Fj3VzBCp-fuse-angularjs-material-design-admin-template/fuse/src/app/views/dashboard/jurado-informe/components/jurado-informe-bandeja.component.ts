...
/** Jurado Informante · Etapa 7 — bandeja de informes finales asignados. */
@Component({
  selector: 'app-jurado-informe-bandeja',
  standalone: true,
    ...
    <div class="page">
      <div class="page-header">
        <div>
          ...
          <h1 class="page-title">Jurado Informante · Informe final</h1>
        </div>
      </div>

      <div class="page-content p-6">
        <p class="text-[12px] text-slate-500 mb-3">
          Informes finales donde integras el Jurado Informante. Evalúa cada uno y observa o da conformidad.
        </p>
        ...
        <div class="data-table">
          <table class="w-full text-sm">
            <thead><tr class="text-slate-400 text-left">
              ...
              <th class="py-2 px-3 font-medium">Rol</th>
            ...
            <tbody>
              @for (r of rows(); track r.tesisId) {
                <tr class="border-t border-slate-100">
                  ...
                  <td class="py-2 px-3">
                    @if (r.presidente) { <span class="px-2 py-0.5 rounded text-[11px] font-bold bg-[#FDF6F7] text-[#8C1D2E]">Presidente</span> }
                    @else { <span class="text-[11px] text-slate-400">Miembro</span> }
                  </td>
                  <td class="py-2 px-3">
                  ...
                  <td class="py-2 px-3 text-right">
                    <button mat-flat-button color="primary" class="!h-8 !text-xs" (click)="abrir(r)">
                      <mat-icon svgIcon="clipboard-check" class="size-3.5 mr-1" /> {{ r.miEstado === 'CONFORME' ? 'Ver' : 'Evaluar' }}
                    </button>
                  </td>
                </tr>
              }
              @if (!rows().length && !loading()) {
                <tr><td colspan="5" class="py-6 text-center text-sm text-slate-400">No tienes informes finales asignados.</td></tr>
              }
            </tbody>
          </table>
        </div>
...
export class JuradoInformeBandejaComponent implements OnInit {
  ...
  abrir(r: any): void { this._router.navigate(['/admin/jurado-informe', r.tesisId]); }

  estadoLabel(e: string): string { return e === 'CONFORME' ? '✓ CONFORME' : e === 'OBSERVADO' ? '⚑ OBSERVADO' : 'POR EVALUAR'; }
