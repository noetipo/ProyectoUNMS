          ...
          @for (item of modules; track item.id; let i = $index) {
            <tr [class.anim-flash]="item.id === highlightedId">
              ...
              <td>
                ...
                >
                  <span class="status-dot" [class.status-dot--active]="item.status" [class.status-dot--inactive]="!item.status"></span>
            ...
            <tr>
              <td colspan="8" class="text-center">
                <div class="table-empty">
                  ...
                  <p class="table-empty__text">No hay módulos registrados</p>
                  <p class="table-empty__subtext">Crea el primer módulo con el botón "Nuevo módulo"</p>
                </div>
              </td>
            </tr>
          }
    ...
    <div class="md:hidden divide-y divide-slate-100">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <div class="flex-1 min-w-0">
              ...
              @if (item.parentModule?.title) {
                <p class="text-xs text-slate-500 mt-1">{{ item.parentModule!.title }}</p>
              }
              @if (item.link) {
      ...
      @empty {
        <div class="flex flex-col items-center gap-2 py-16 anim-fade-in">
          <mat-icon svgIcon="layout-grid" class="size-10 text-slate-200" />
          <p class="text-sm text-slate-400">No hay módulos registrados</p>
        </div>
      }
    </div>
  `,
