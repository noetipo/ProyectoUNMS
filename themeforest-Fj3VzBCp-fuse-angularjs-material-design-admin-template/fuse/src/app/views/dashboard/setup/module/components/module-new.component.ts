  ...
  template: `
    <div class="form-dialog w-[560px]" [class.anim-shake]="shaking()">
      ...
      <div class="form-dialog__header">
        <div>
          <h2 class="form-dialog__title">Nuevo módulo</h2>
          <p class="form-dialog__subtitle">Completa la información del módulo</p>
        </div>
          ...
          <div class="form-section" style="animation-delay: 40ms">
            ...
            <mat-form-field appearance="outline" class="w-full" subscriptSizing="dynamic">
              <mat-select formControlName="parentModuleId" placeholder="Seleccionar módulo padre">
                @for (pm of parentModules; track pm.id) {
...
export class ModuleNewComponent implements OnInit {
  ...
  dialogRef = inject(MatDialogRef<ModuleNewComponent>);
  form!: UntypedFormGroup;

  protected shaking = signal(false);

  constructor(@Inject(MAT_DIALOG_DATA) public parentModules: ParentModule[]) {}

  ngOnInit(): void {
    this.form = this._fb.group({
      parentModuleId: ['', Validators.required],
